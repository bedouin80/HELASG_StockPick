...
module.exports = function(node) {
    ...
    node.on("input", function(msg,send,done) {
        ...
        update(+1);
        jq.stdout.pipe(JSONStream.parse().on('data', function (obj) {
            msg.payload = obj;
            //node.send(msg);
            send(msg);
        }));
        jq.stderr.on('data', function (data) { /* node.warn(data); */ done(data); });
        jq.on('close', function (code) { update(-1); done(); });
        jq.on('error', function (code) { /* node.warn(code); */ done(code); });
